...
## 2026-06-27 본문 프롬프트 전환력 강화 — 전문가개입 실타래 + 방문해야만 풀리는 CTA + '왜 곳마다 다른가' 각
- Change-Fingerprint: 2df2e883330b97a4
- Gate Result: PASS — type-check 0 | P0 | PASS · build 0(✓ 정적생성 정상) | P0 | PASS · lint 0(articlePrompt.ts) | P1 | PASS · articlePrompt.test 7/7 | P1 | PASS · 핵심기능삭제 0(기존 지침·게이트·키워드규칙·매장안내 전부 보존, 추가만 함) | P0 | PASS · 민감정보출력 0 | P0 | PASS.
- 마케팅콘텐츠 게이트: 과장/금지표현 0(비교·비방 절대금지 명시 유지, 의료법 안전 규칙 불변) | P0 | PASS · CTA 포함(오히려 강화) | P1 | PASS.
- 배경: 사용자가 마케팅 강의(브랜드 블로그=상업적 전환 목적, 정보로 이탈 방지 + 정보마다 "우리가 이렇게 해결"로 연결 → 연락/방문) 프레임으로 현재 글 평가 요청. 실제 생성 글 4편(_verify: article2/re_article/contacts_article/re_frames_article) + articlePrompt.ts/promoPrompt.ts 분석. 강의 프레임 기준 ~7/10 진단: 뼈대 OK, 약점 3개(전환 실타래 끝에만 몰림·"왜 곳마다 다른가" 최강레버 부재·방문이유 막연).
- 수정(사용자 승인 후 — "다 바꾸지말고 예시 보여주고 적용", 1파일 외과적 추가만):
  - `articlePrompt.ts` 3-1 전개원칙: "전문가 개입 가치(글 전체에 분산)" 1줄 추가 — 중간 정보 소제목 1~2곳에 "혼자 가늠 어렵고 검안·피팅에서 갈린다" 신호, 헤징연속·복붙·끝섹션몰이 금지 가드 포함.
  - 결론 지침: 방문 이유를 "직접 와야만 풀리는 것"(직접착용·PD/광학중심 측정·피팅)으로 구체화 규칙.
  - 신규 3-2(선택): 주제 맞을 때만 저가/온라인 대비 "제대로 맞추는 가치" 1단락, 비교·비방 절대금지 명시.
  - 출력 [결론] 항목 + 최종 체크리스트 20·21번 보강(위 1·2 자가검증).
- 검증: type-check/lint/build green. articlePrompt.test 7/7 PASS(기존 회귀 없음). 변경은 프롬프트 텍스트 추가뿐(제어흐름·시그니처 불변).
- 미검증: 실제 글 생성 전/후 비교 E2E 미실행(사용자 승인 후 진행 예정). promoPrompt.ts는 동일 패턴 미적용(정보70/홍보30 구조라 우선순위 낮음 — 보류).
- 검수자: 메인 직접(강의프레임 분석·실글 4편 검토 + 코드확인 + 전게이트). 미커밋(push 사용자 승인 대기).

## 2026-06-27 전환력 후속 — 변경1 강제력 강화 + sanitizer 금지어 갭(정답·가장) 근본수정 + 회귀테스트
- Change-Fingerprint: 63b241dc06190b7b
- Gate Result: PASS — type-check 0 | P0 | PASS · build 0(✓ 31/31 정적생성) | P0 | PASS · lint 0(3파일) | P1 | PASS · test 39/39(articlePrompt 7·medicalLawSanitizer 12·contentValidator 20) | P1 | PASS · 핵심기능삭제 0(기존 치환·매장블록보호·복합어가드 전부 보존, 추가만) | P0 | PASS · 민감정보출력 0 | P0 | PASS.
- 마케팅콘텐츠 게이트: 과장/금지표현 0(비교·비방 금지 유지) | P0 | PASS.
- 배경: 사용자 "다해라" — (1)변경1 강화 (2)sanitizer 근본수정 (3)변경3 데모 전부 진행. 직전 데모에서 워싱 후에도 "정답"(초안 잔여)·"가장"(워싱 LLM이 "특히 흔한"→"가장 흔한"으로 재유입) 미교정 = validator는 잡는데 sanitizer 사전에 없어 자동수정 불가(두 치환맵 desync) 갭 확인.
- 수정:
  - `articlePrompt.ts`: 변경1을 "선택"→"최소 1회 필수"로 강화(끝 "매장에서 확인" 섹션 제외, 중간 본문 소제목에서). 체크리스트 20번 동기화.
  - `medicalLawSanitizer.ts`: PHRASE에 `/정답(은|이)?\s*아니/`→"능사는 아니"(구문 우선), WORD에 `/정답/`→"답"(잔여) + `/가장(?=\s)/`→"특히"(최상급만; "가장자리"·"가장무도회" 등 무공백 복합어는 lookahead로 보호). Pass3(워싱 후 재-sanitize)가 이 사전으로 워싱 재유입분까지 자동 청소 = 사용자가 말한 "재청소"는 기존 Pass3 + 사전 동기화로 닫힘.
  - `medicalLawSanitizer.test.ts`: 회귀 4건 추가(정답구문→능사·잔여정답→답·가장흔한→특히·가장자리 보호).
- 검증: type-check/lint/build green. 단위 39/39. **자가치유 E2E 실증**: 수정 전 원본 초안(정답·가장·차근차근 포함)을 /api/article/wash에 수동손질 없이 재투입 → 결과 prohibited:[]·정답 false·가장흔한 false·차근차근 false(washReport absolute:1 = Pass3 자동교정). 라이브 :3100.
- 미검증/별개이슈: 변경3 데모 2회 모두 **키워드 드리프트**(요청 mainKeyword를 본문이 무시: "콘택트렌즈 빛번짐"→흐림, "안경 가격 차이"→빛번짐). 라우트는 입력 키워드를 그대로 프롬프트에 전달(기록 메타 일치)·writeArticle 캐시 없음 확인 → 작성 LLM이 title/keyword보다 리서치로 흐른 것으로 추정. topic 명시 앵커 적용한 3차 데모 생성 중(결과 대기). 이는 이번 변경과 무관한 선재 키워드준수 버그 → 별도 TASK 권장.
- 검수자: 메인 직접(코드확인 + 단위 39 + 자가치유 라이브 E2E + 전게이트). 미커밋(push 사용자 승인 대기).

## 2026-06-27 키워드 드리프트 근본수정(A안) — 리서치 표류 감지 후 제목 논지 폴백
- Change-Fingerprint: 8027286cc104186c
- Gate Result: PASS — type-check 0 | P0 | PASS · build 0(✓ 31/31) | P0 | PASS · lint 0(3파일) | P1 | PASS · test 7/7(researchRelevance 신규) | P1 | PASS · 핵심기능삭제 0(리서치/형태소/프롬프트 경로 보존, 표류시에만 분기) | P0 | PASS · 민감정보출력 0 | P0 | PASS.
- 배경: 변경3 데모 3/3 키워드 드리프트(요청 mainKeyword 무시) 원인 규명. 라우트에 임시 덤프 삽입→실경로 캡처(진단 후 제거, 소스 잔존 0 확인).
- 근본원인(확정, 덤프 증거): effectiveThesis는 정상("싼 안경과 비싼 안경 진짜 차이")·competitorMorphology는 unavailable(무죄)인데 **researchData(Perplexity) 1653자가 전부 "기름막/세척/코팅" = 가격 0줄**. 메커니즘: 리서치 프롬프트의 "물리적 현상만 조사(SEO/메타 금지)" 가드가, 물리현상이 아닌 가격·비교 키워드를 가장 가까운 물리 안경주제(세척)로 디폴트시킴 → 본문 프롬프트 "조사자료 기반"이 그 자료를 충실히 따라 제목 이탈.
- 수정(사용자 승인 "a로 가자"):
  - 신규 `src/lib/ai/researchRelevance.ts`(순수): `extractDistinctiveTokens`(도메인 일반어 안경·렌즈·눈 등 제외, 2자+ 변별 토큰), `isResearchOnTopic`(자료<40자/변별토큰0=판단보류 true; 변별토큰 1개+ 등장=on-topic — 보수적 오폐기 최소화), `OFF_TOPIC_RESEARCH_NOTE`(제목 논지 충실+수치 날조 금지 가드문).
  - `route.ts`: researchData를 표류 판정으로 분기 — on-topic이면 원자료, off-topic이면 NOTE로 대체하고 researchStatus="empty". import 추가.
  - 테스트 7건(세척자료=off-topic·정상현상=on-topic·빈자료/일반어=보류·토큰추출).
- 검증: tsc/lint/build green, 단위 7/7. **라이브 E2E PASS** — 동일 가격 주제(_verify/change3_fixed.json, 2069자) 재생성 결과 본문이 전부 "안경 가격 차이·렌즈 설계/코팅·검안/맞춤"으로 정렬(드리프트 흔적 0: 기름막·세척·빛번짐 모두 미출현). 직전 동일 입력 3회는 세척/코팅으로 표류했으나 가드 적용 후 제목 주제 유지 확인. 부가로 변경1(중간섹션 전문가신호)·변경2(방문해야만 CTA)·변경3(저가 vs 제대로 가치)까지 한 글에서 동시 발현.
- 영향 메모: 약한 표류(콘택트 빛번짐→흐림)는 변별토큰 부분일치 시 통과될 수 있음(보수적 설계 — 심한 표류 우선 차단). 정상 주제는 변별토큰 등장으로 그대로 통과(회귀 위험 낮음).
- 검수자: 메인 직접(실경로 덤프 진단 + 순수함수 단위 + 전게이트). 라이브 E2E 대기. 미커밋(push 사용자 승인 대기).
