...
- 모든 키워드는 이 카테고리(${category.name})의 제품·주제 범위 안에서만 만든다. 다른 카테고리 제품(예: 안경테 카테고리에 콘택트렌즈·선글라스·누진렌즈 / 콘택트렌즈 카테고리에 안경테 등)은 절대 넣지 않는다.
- 카테고리 안에서 검색자는 카테고리명 대신 증상, 상황, 선택 기준으로 검색하므로 그 범위 안에서 다양하게 확장한다.
