...
- 군집 쏠림 절대 금지: 특히 안경 관리/이야기 주제에서 "피팅·코받침·조정·흘러내림"(착용 보정) 한 군집에 절반 이상 몰지 마세요. 세척·김서림·보관·렌즈 흠집·스크래치·코팅·안경닦이처럼 다른 군집으로 반드시 나눠 최소 4가지 이상 서로 다른 소재가 나오게 하세요.
...
- 위 [네이버 검색광고 조회]에 실제 월 검색량이 잡힌 검색어를 메인 키워드로 적극 채택하세요. 실측 검색량이 있는 검색어가 검색량 미확인 조어보다 훨씬 좋습니다. 롱테일은 "검색량이 0이 아닌 것 중에서" 블로그 발행수가 적은 것을 고르는 것이지 아무도 안 찾는 조어를 지어내는 게 아닙니다.
...
- 헤드 뒤에 어색한 수식어(고온·환경·차량 같은)를 붙여 아무도 검색하지 않는 조합을 만들지 마세요. 실제 관측된 나쁜 예: "안경보관 고온" "안경착용감 코" "안경관리 방법" "코패드교체 소재" "안경흘러내림 피팅" — 이런 비검색 조합은 절대 만들지 마세요. 각 키워드는 "이 두 단어를 그대로 붙여 검색하는 사람이 있는가"를 자문해 아니면 버리세요.
- 실검색어 고정(중요): 메인 키워드는 가능한 한 위 [네이버 검색광고 조회] 목록에 실제로 등장한 검색어 또는 그 머리단어로 잡으세요. 그 목록은 사람들이 실제로 검색한 실측 데이터입니다. 목록에 없는 단어를 새로 붙여 만든 조합은 실제 검색량이 0일 가능성이 높습니다. 목록의 실볼륨 검색어를 우선 활용하세요.
