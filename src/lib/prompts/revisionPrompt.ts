...
export function buildRevisionPrompt(params: {
  ...
  if (validation.languageRisk?.formatViolations?.length) {
    problemLines.push(
      `- 형식 위반 수정(필수): ${validation.languageRisk.formatViolations.join(", ")} — 쉼표가 남발된(한 문장 3개 이상) 부분만 연결 어미로 풀거나 문장을 나누세요. 호흡에 필요한 1~2개는 둬도 됩니다(표 안·천단위 숫자 예외).`
    );
  }
  ...
  if (validation.languageRisk?.templateLeaks?.length) {
    problemLines.push(
      `- 지침 단어 노출 수정(필수): ${validation.languageRisk.templateLeaks.join(
        " / "
      )} — (아래 따옴표 안 단어들은 '제거 대상'입니다. 본문에 새로 쓰지 마세요.) "신호" "넘겨짚을" "판별 축" "확인 순서" 같은 작성 지침용 단어를 소제목·표 머리글에서 빼고 실제 사람이 쓰는 말로 바꾸세요. 표 머리글은 비교 대상을 그대로 가리키는 말로 쓰세요(예: "뿔테 / 금속테" "증상 / 대처법"). "집에서 보이는 모습" 같은 억지 구분은 만들지 마세요. 표의 행 구분과 내용은 그대로 두고 머리글 문구만 자연어로 교체하세요.`
    );
  }
...
4. 자연스러운 문장 흐름 — 한 문장마다 빈 줄로 토막 내지 말고 관련 문장 3~5개를 한 문단으로 묶되(빈 줄은 문단 사이에만), 단문을 그냥 붙이지 말고 연결 어미(~해서·~다 보니·~는데·~으면서)와 앞 문장을 이어받는 흐름으로 매끄럽게 이으세요. 사실 나열·단답형은 반드시 흐르는 문장으로 고치세요.
5. 숫자 나열(1. 2. 3.) 대신 문장으로 풀어서 작성
6. 쉼표(,)는 제한적으로만 — 호흡에 필요한 곳 한 문장 1~2개 이내. 한 문장에 3개 이상 몰리면 연결 어미로 풀거나 문장을 나누세요(남발 금지).
7. 비교·스펙·상황 구분 정보가 있는데 표가 없으면 표로 정리해도 좋습니다. 단 표에 넣을 실제 내용이 없으면 억지로 만들지 마세요(빈 표·내용 날조 금지).
...
- 쉼표(,)는 남발 금지(호흡에 필요한 1~2개는 허용). 숫자 나열(1. 2. 3.) 금지.
...
// 자연어 다듬기 패스: 어색한 압축 조어("시선 자리"류)만 사람 말투로 푼다.
// 하드코딩 단어 목록이 아니라 "소리 내어 읽기" 원칙으로 부류 전체를 잡되,
// 진짜 안경 용어·키워드·구조·의미는 보존한다. (설계: docs/designs/naturalness-polish-pass.md)
export function buildNaturalnessPolishPrompt(params: {
  originalContent: string;
  mainKeyword: string;
  subKeyword1?: string;
  subKeyword2?: string;
  ...
  const { originalContent, mainKeyword, subKeyword1, subKeyword2, charCount = 2000 } = params;

  return `당신은 한국어 자연스러움만 다듬는 에디터입니다. 글을 새로 쓰지 말고, 딱 한 가지 문제만 고치세요.

[유일한 임무 — 어색한 압축 조어 풀기]
- 본문을 한 문장씩 소리 내어 읽으세요. 안경사가 손님 앞에서 입으로 말하지 않을 표현이면 사람이 말하듯 풀어 쓰세요.
- 대상은 "명사 + 명사를 억지로 붙여 만든 그럴듯한 조어"입니다. 예: "시선 자리" → "시선이 닿는 위치", "피팅 자리" → "피팅 위치".
- 이건 단어 목록 암기가 아닙니다. 판별 기준은 오직 하나 — "사람이 실제로 입으로 그렇게 말하는가". 어색하면 풀고, 자연스러우면 그대로 둡니다.

[절대 보존 — 건드리지 말 것]
- 실제 안경 전문용어는 그대로 두세요: 누진대·단초점·굴절력·아이포인트·도수·난시·근시·원시·수차 등. 이런 용어를 장황하게 풀지 마세요(장황함도 어색함입니다).
- 키워드 원형 유지: 메인 "${mainKeyword}"${subKeyword1 ? `, 서브 "${subKeyword1}"` : ""}${subKeyword2 ? `, "${subKeyword2}"` : ""}.
- 매장 안내 블록(주소·운영시간·주차·네이버 플레이스)은 한 글자도 바꾸지 마세요.
- "## " 소제목·표·어미 계열·의미는 그대로 유지하세요. 단 한 문장마다 빈 줄로 토막 나 있으면 관련 문장을 한 문단으로 묶어 흐름을 살려도 됩니다.

[금지]
- 쉼표(,) 남발 금지(호흡에 필요한 1~2개는 허용). 문장을 장황하게 늘리지 마세요(짧고 자연스럽게만).
- 어색하지 않은 문장은 절대 손대지 마세요(과교정 금지). 고칠 게 없으면 원문을 그대로 출력하세요.
- 글 길이는 ${charCount}자 내외를 유지하세요.
