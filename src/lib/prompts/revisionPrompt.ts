  ...
  if (validation.languageRisk?.templateLeaks?.length) {
    problemLines.push(
      `- 지침 단어 노출 수정(필수): ${validation.languageRisk.templateLeaks.join(
        " / "
      )} — "신호" "넘겨짚을" "판별 축" "확인 순서" 같은 작성 지침용 단어를 소제목·표 머리글에서 빼고 실제 사람이 쓰는 말로 바꾸세요. 표 머리글은 비교 대상을 그대로 가리키는 말로 쓰세요(예: "뿔테 / 금속테" "증상 / 대처법"). "집에서 보이는 모습" 같은 억지 구분은 만들지 마세요. 표의 행 구분과 내용은 그대로 두고 머리글 문구만 자연어로 교체하세요.`
    );
  }
