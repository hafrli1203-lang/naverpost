...
- "[SCENE:detail] " : 안경 부품·제품 디테일 컷. 글 주제가 코받침·렌즈 두께/코팅·테 소재·콘택트 같은 "부품 디테일"이면 이 태그를 쓰세요. 이 컷은 AI가 그리지 않고 실제 사진으로 서빙되므로 초접사 매크로도 왜곡 없이 나옵니다(부품 클로즈업 환영). 안경테 진열을 적당한 거리에서 여러 개 보여주는 컷도 이 태그로 OK.
규칙:
...
- ★ 매장 전경 와이드샷([SCENE:interior]/[SCENE:exterior])은 본문 중간에 뜬금없이 넣지 마세요. 매장 와이드는 글의 "마무리"(매장 안내·방문 권유) 구간에 1장 정도만 배치하고, 도입부·본문 중간 구간은 라이프스타일·착용·개념 컷으로 채우세요. 매장 안 장면이 필요하면 중간에는 [SCENE:fitting](상담·조정)처럼 사람이 있는 장면을 쓰고, 사람 없는 빈 매장 와이드는 마무리로 미루세요.
...
★ 안경은 좌우 대칭·비왜곡(찌그러진 테·녹은 코받침·짝짝이 렌즈 금지). 코받침·힌지·나사 같은 작은 부품의 초접사 매크로가 필요하면 반드시 [SCENE:detail] 태그로 쓰세요(실제 사진으로 서빙되어 왜곡 없음). 태그 없이 AI로 그리는 부품 초접사 매크로는 만들지 마세요(왜곡) — 그 경우엔 사람이 콧등을 만지는 컷·안경을 정면 평면으로 둔 컷·거울 앞 착용 컷으로 대체하세요.
★ 빈 매장 전경 와이드샷은 본문 중간에 넣지 말고 마무리(매장 안내) 구간에만 1장.
